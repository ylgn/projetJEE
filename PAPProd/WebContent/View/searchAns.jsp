...
<!DOCTYPE html>
<% List<ObjectPAPForClient> obs = (ArrayList<ObjectPAPForClient>) request.getAttribute("listResult");
...
<body>
	...
	<div class="section">
		...
		<div class="container">
			...
			<div class="row">
	<% 	
		String action = "../PAPProd/CAppli";
	
		for(ObjectPAPForClient o : obs)
    {
    
    out.print("<form id='form' action ="+action+" method='post'>");
    
    if (o.getDescriptionObject().isEmpty()){%>
    	Désolé, aucuns éléments ne correspondent à votre recherche.
   <% }
    %>
  ...
  <tbody>
    <tr>
      ...
      <td><%out.print(o.getNameObject());%></td>
      <td><%out.print(o.getDescriptionObject()); %></td>
      <td><%out.print(o.getPriceObject());%></td>
      <td><%
       int obn = o.getNumObject();
       
        out.print("<br/>");
        out.print("<input type='hidden' name='objectNumber' value ="+obn+"/>");
        out.print("<input type='hidden' name='function' value ='buy' />");
        
        if(!o.getIsSelled()){
        	out.print("<button class='primary-btn'>Acheter</button>");
        	out.print("<br/>");
        
        }else if(!currentSession.getAttribute("connected").equals("ok")){
        	out.print("Connectez-vous pour acheter l'objet");
        }
